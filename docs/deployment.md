...
The local demo still uses SQLite and one Python worker process. The repository now
...
flowchart LR
    ...
    P --> C
    C --> R[SMTP alerts + draft PR + checkpoints]
...
| Component | Behavior |
| --- | --- |
| `quality/postgres.py` | Shared journal, bound SQL parameters, pooled connections and atomic `FOR UPDATE SKIP LOCKED` job claims. |
...
| Controller Deployment | One process owns exports, monitoring, notification, repair and checkpoint scheduling. It has a persistent artifact volume. |
...
Model work that exceeds the 600-second termination grace period may be reclaimed.

## Deploy when a cluster is available

Prerequisites: a PostgreSQL database dedicated to this journal, a reachable Phoenix
instance, an image registry, a Kubernetes context and a default storage class.
...
1. Build and push the image using your registry. Pin the same image digest for the
   API, controller and evaluator deployments in `deploy/kubernetes/kustomization.yaml`.
...
3. Supply two Secrets through your organization's secret manager. For a private
   ...
   - Controller: `GITHUB_TOKEN`, `QUALITY_SMTP_USERNAME`, `QUALITY_SMTP_PASSWORD`.
     Scope the GitHub credential to the target repository. The controller is the
     only deployment that receives publication/SMTP credentials.
...
scale API/controller/evaluators to zero, allow existing jobs to drain, update the
...
within one Deployment overlapping; it does not coordinate three Deployments.
...
This is a concrete path for horizontal **evaluation-worker** scaling while preserving
