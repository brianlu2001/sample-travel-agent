...
function renderActivity(){
 ...
 const title=x.kind==='repair'?(x.payload.pr_url?'PR #'+x.payload.pr_url.split('/').pop()+' · '+(names[x.payload.metric]||'Agent improvement'):(names[x.payload.metric]||'Agent improvement')+' · '+x.id.slice(0,8)):x.payload.source==='checkpoint'?`${names[x.payload.metric]||x.payload.metric} · Checkpoint regression`:`${names[x.payload.metric]||x.payload.metric} below ${pct(x.payload.threshold)}`;
